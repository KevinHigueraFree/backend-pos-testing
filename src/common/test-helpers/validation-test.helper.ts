 ...
 */
export const testInvalidIdValidation = async (id: any, expectedMessage: string = 'Invalid ID') => {
  const idValidationPipe = new IdValidationPipe();

  // Act & Assert - Probar el pipe directamente
  await expect(idValidationPipe.transform(id, { type: 'param', data: 'id' })).rejects.toThrow(
    BadRequestException
  );

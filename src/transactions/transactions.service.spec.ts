...
describe('TransactionsService', () => {
    ...
    // Mock del transactionEntityManager que se usa dentro de la transacción
    ...
    beforeEach(async () => {
        ...
        transactionContentsRepository = module.get<Repository<TransactionContents>>(getRepositoryToken(TransactionContents));
        productRepository = module.get<Repository<Product>>(getRepositoryToken(Product))
        couponsService = module.get<CouponsService>(CouponsService)

        // Configurar el mock de transaction para que ejecute el callback con el transactionEntityManager
        // y retorne el valor que retorna el callback
    ...
    describe('create', () => {
        it('should create a transaction successfully without coupon', async () => {
            ...
            expect(mockTransactionEntityManager.findOneBy).toHaveBeenNthCalledWith(1, Product, { id: productId1 })
            expect(mockTransactionEntityManager.findOneBy).toHaveBeenNthCalledWith(2, Product, { id: productId2 })
        ...
        it('should create a transaction successfully with coupon', async () => {
            ...
            expect(mockProductRepository.manager.transaction).toHaveBeenCalled()
            expect(mockTransactionEntityManager.findOneBy).toHaveBeenCalledWith(Product, { id: productId })
            expect(mockTransactionEntityManager.save).toHaveBeenCalled()
            expect(result).toBe("Sale storaged correctly")
        })
        ...
        it('should throw NotFoundException when product does not found', async () => {
            ...
            // Act & Assert
            await expect(transactionsService.create(createTransactionDto)).rejects.toThrow(NotFoundException)
        ...
        it('should throw BadRequestException when product inventory is insufficient', async () => {
            ...
            const createTransactionDto: CreateTransactionDto = {
                ...
                contents: [
                    {
                        productId: productId,
                        quantity: 100, // Cantidad mayor al inventario disponible (25)
                        price: 1500
                    }
                ]
            }
            ...
            // Act & Assert
            await expect(transactionsService.create(createTransactionDto)).rejects.toThrow(BadRequestException)
        ...
        it('should update product inventory correctly', async () => {
            ...
            // Assert
            expect(mockTransactionEntityManager.findOneBy).toHaveBeenCalledWith(Product, { id: productId })
            // Verificar que el inventario del producto se actualizó en memoria
        ...
        it('should calculate discount correctly when coupon is applied', async () => {
            ...
            // Assert
            expect(mockCouponsService.applyCoupon).toHaveBeenCalledWith(couponName)
            // Verificar que se guardó la transacción con el descuento correcto
            // El servicio guarda la transacción varias veces, la última debe tener el descuento aplicado
            const savedCalls = mockTransactionEntityManager.save.mock.calls
            // Buscar la última llamada que guarda una transacción (no TransactionContents)
            const transactionSaves = savedCalls.filter(call =>
                call[0].hasOwnProperty('total') &&
                call[0].hasOwnProperty('discount')
            )
        ...
        it('Should return one transaction found by id successfully', async () => {
            ...
            it('Should return NotFoundException, not found the transaction by id', async () => {
                ...
                try {
                    ...
                    expect(error).toBeInstanceOf(NotFoundException);
                    expect(error.response.message).toContain(`The Transaction with ID ${transactionId} does not found`);
                }
                expect(transactionRepository.findOne).toHaveBeenCalled()
                expect(transactionRepository.findOne).toHaveBeenCalledWith({
    ...
    describe('remove', () => {
        it('Should return removed transaction succesfully', async () => {
            ...
            expect(mockProductRepository.save).toHaveBeenCalledWith({ ...product1, inventory: expectedInventory1 })
            expect(mockProductRepository.save).toHaveBeenCalledWith({ ...product2, inventory: expectedInventory2 })
            expect(mockTransactionContentsRepository.findOneBy).toHaveBeenCalledTimes(2)
            expect(mockTransactionContentsRepository.findOneBy).toHaveBeenCalledWith({ id: transactionContentId1 })
            expect(mockTransactionContentsRepository.findOneBy).toHaveBeenCalledWith({ id: transactionContentId2 })

...
describe('CouponsController (e2e) - Integration Tests', () => {
  ...
  afterEach(async () => {
    // Clean all coupons after each test
  ...
  describe('POST /coupons', () => {
    it('Should return coupon created succesfully', async () => {
      ...
      expect(couponInDB?.expirationDate.toISOString().split('T')[0]).toBe(
  ...
  describe('GET/coupons:id', () => {
    ...
    it('Should return 404, when does not found coupon', async () => {
      ...
      const response = await request(app.getHttpServer()).get(`/coupons/${couponId}`).expect(404);
  ...
  describe('POST /apply-coupon', () => {
    ...
    it('Should return 422, when coupon is unprocessable', async () => {
      ...
      couponCreateDto.expirationDate = subDays(new Date(), 1);

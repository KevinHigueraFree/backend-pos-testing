...
describe('TransactionsController (e2e) - Integration Tests', () => {
  ...
  // AFTER ALL TESTS: Close the application and database connection
  afterAll(async () => {
    // Close TypeORM connection first
    if (dataSource && dataSource.isInitialized) {
      try {
    ...
    // Then close NestJS application
    if (app) {
      try {
        await app.close();
      } catch (error) {
        // Ignore errors when closing app
      }
    }
  });

  describe('POST /transactions', () => {
    it('Should return transaction created successfully (without coupon)', async () => {
      // Arrange
      ...
      // Verify that the transaction was saved in the database
      ...
      // Verify that products have updated inventory
      ...
      // Sort contents by product ID for consistent order
    ...
    it('Should return transaction created successfully (with valid coupon)', async () => {
      // Arrange
      const transactionRepository = dataSource.getRepository(Transaction);
    ...
    it('Should return 400 when product inventory is insufficient', async () => {
      ...
      // Assert
      ResponseTestHelper.expectBadRequest(response, [
        `The product ${product.name} exced the enable quantity`,
      ]);
    });
    it('Should return 422 when coupon is expired', async () => {
      ...
      const couponSaved = await couponRepository.save({
        ...createCouponDto,
    ...
    it('Should return 404 when coupon does not exist', async () => {
      ...
      ResponseTestHelper.expectNotFound(
        response,
        `The Coupon with name: ${couponName} does not found`
      );
    });
  });
  ...
  describe('DELETE /transactions:id', () => {
    it('Should return 200 when transaction was removed successfully', async () => {
      ...
      expect(response.body.message).toBe(
        `The Transaction with ID ${transactionSaved.id} was removed`
      );
    });
    it('Should return 404 when transaction does not found', async () => {
      // Arrange
